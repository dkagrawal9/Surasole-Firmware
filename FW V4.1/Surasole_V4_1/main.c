...
#define Data_mode        Mode_5_sensor
...
char * name_R = "AT+NAMESurasole-MR";
char * name_L = "AT+NAMESurasole-ML";

//char * name_R = "AT+NAMESurasole-LR";
//char * name_L = "AT+NAMESurasole-LL";

//char * name_R = "AT+NAMESurasole-43-44R";
//char * name_L = "AT+NAMESurasole-43-44L";
char * rst  = "AT+RST";
...
//////////// SLEEP ////////////
volatile uint16_t Wait_sleep = 60;// Second
...
#define MUX_S0  PORTD4
#define MUX_S1  PORTD5
#define MUX_S2  PORTD6
const int pin_cmd[3] = { MUX_S0, MUX_S1,  MUX_S2}; // S0 >> PD4, S1 >> PD5, S2 >> PD6

#define EN_MUX   PORTD &= ~(1 << PORTD3);
#define DIS_MUX  PORTD |=  (1 << PORTD3);

#define EN_PWRC   PORTB |=   (1 << PORTB2);
#define DIS_PWRC  PORTB &=  ~(1 << PORTB2);

#define EN_CHG_PUMP   PORTD |=   (1 << PORTD7);
#define DIS_CHG_PUMP  PORTD &=  ~(1 << PORTD7);


#define VREF_3V3   PORTB |=  (1 << PORTB0);
#define VREF_BAT   PORTB &=  ~(1 << PORTB0);

uint16_t MUX_ADC_CH(uint8_t pin)
{
	for (int i=0; i<3; i++)
	{
		if (pin & (1<<i)) // HIGH
	...
	_delay_us(20);
	return ADCsingleREAD(0);  // CH0 >> Read ADC
}
...
void init_usart()
{
	...
	UCSR0C &= ~((1 << UMSEL01) |  (1 << UMSEL00)); // enable asynchronous USART
...
void disable_uart()
{
    ...
    UCSR0B &= ~((1 << TXEN0) | (1 << RXEN0));
		...
		DDRD  = 0b11111011; // UART and MUX Control
		PORTD &= ~(1 << PORTD0);
		PORTD &= ~(1 << PORTD1);
}
...
{
	//PORTB = 0b00000001; // PB3 >> Mosfet BLE // PB2 >> PWRC
	DIS_PWRC; // PWRC LOW LEVEL
	//EN_CHG_PUMP;
	_delay_ms(200);
	int tr_n;
...
ISR(INT0_vect)
{
	...
	DDRD  = 0b11111010; // UART and MUX Control
	init_usart();
	init_timer();
	start_timer();
...
int main()
{
	...
	DDRB  = 0b11111101; // PB2 >> PWRC // PB0 >> Analog switch
	DDRD  = 0b11111010; // UART and MUX Control
	...
	{
		adc_batt = ADCsingleREAD(1); // Read Battery voltage
	...
	while(1)
	{
		...
		if(_en_send)
		{
			...
			if(FOOT_SEL==RIGHT_FOOT)
			{
			    EN_MUX;
				_delay_us(20);
				adc_raw[0] = MUX_ADC_CH(0); // FSR 0
				adc_raw[1] = MUX_ADC_CH(1); // FSR 1
				adc_raw[2] = MUX_ADC_CH(2); // FSR 2
				adc_raw[3] = MUX_ADC_CH(3); // FSR 3
				adc_raw[4] = MUX_ADC_CH(4); // FSR 4
				adc_raw[5] = MUX_ADC_CH(5); // FSR 5
				adc_raw[6] = MUX_ADC_CH(6); // FSR 6
				adc_raw[7] = MUX_ADC_CH(7); // FSR 7
				_delay_us(20);
				DIS_MUX;
				////////////////////////////// CLEAR TIMER ///////////////////////////
				...
				//{
					//timer1_second=0;
				//}
				if(adc_raw[1]>= threshold || adc_raw[2]>= threshold || adc_raw[4]>= threshold || adc_raw[5]>= threshold || adc_raw[7]>= threshold) // 5 sensor
				{
					timer1_second=0;
				}
				/////////////// Check battery voltage //
				adc_batt = ADCsingleREAD(1);
				if(Threshold_Batt < adc_batt)
				{
					per_batt = Calculate_Percent_BaTT(adc_batt); // Percent Batterry
					if(per_batt>100)per_batt = 100;
								...
								data[0] = (adc_raw[1] & 0xff00) >> 8;
								data[1] =  adc_raw[1] & 0xff;
								
								data[2] = (adc_raw[2] & 0xff00) >> 8;
								data[3] =  adc_raw[2] & 0xff;
								
								
								data[4] = (adc_raw[4] & 0xff00) >> 8;
								data[5] =  adc_raw[4] & 0xff;
								
								
								data[6] = (adc_raw[5] & 0xff00) >> 8;
								data[7] =  adc_raw[5] & 0xff;
								
								
								data[8] = (adc_raw[7] & 0xff00) >> 8;
								data[9] =  adc_raw[7] & 0xff;
...
//
				//data[12] = (adc_raw[6] & 0xff00) >> 8;
				//data[13] =  adc_raw[6] & 0xff;
//
				//data[14] = (adc_raw[7] & 0xff00) >> 8;
				//data[15] =  adc_raw[7] & 0xff;
//
			...
			else
			{
			    ...
			    DIS_MUX;
				////////////////////////////// CLEAR TIMER ///////////////////////////
				if(adc_raw[6]>= threshold || adc_raw[5]>= threshold || adc_raw[3]>= threshold || adc_raw[2]>= threshold || adc_raw[0]>= threshold) // 5 sensor
				{
					timer1_second=0;
				}
				/////////////// Check battery voltage //
				...
				if(Threshold_Batt < adc_batt)
				{
					...
					if(per_batt>100)per_batt = 100;
												...
												data[0] = (adc_raw[6] & 0xff00) >> 8;
												data[1] =  adc_raw[6] & 0xff;
												
												data[2] = (adc_raw[3] & 0xff00) >> 8;
												data[3] =  adc_raw[3] & 0xff;	
												
												data[4] = (adc_raw[5] & 0xff00) >> 8;
												data[5] =  adc_raw[5] & 0xff;											
												
												data[6] = (adc_raw[2] & 0xff00) >> 8;
												data[7] =  adc_raw[2] & 0xff;
																								
												data[8] = (adc_raw[0] & 0xff00) >> 8;
												data[9] =  adc_raw[0] & 0xff;
