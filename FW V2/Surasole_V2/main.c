...
/////////// BLE AT COMMAND //////////////
char * name_R = "AT+NAMESurasole-SR";
char * name_L = "AT+NAMESurasole-SL";
char * rst  = "AT+RST";
		...
		///////////// R /////////////////
			if(_en_send)
			{
				...
				if(FOOT_SEL==RIGHT_FOOT)
				{
					...
					adc_raw[2] = ADCsingleREAD(3); // FSR 2
					...
					data[2] = (adc_raw[0] & 0xff00) >> 8;
					data[3] =  adc_raw[0] & 0xff;
					...
					data[6] = (adc_raw[4] & 0xff00) >> 8;
					data[7] =  adc_raw[4] & 0xff;

				
					data[8] = (adc_raw[3] & 0xff00) >> 8;
					data[9] =  adc_raw[3] & 0xff;
				...
				{
					adc_raw[0] = ADCsingleREAD(3); // FSR 0
					adc_raw[1] = ADCsingleREAD(1); // FSR 1
					...
					data[0] = (adc_raw[4] & 0xff00) >> 8;
					data[1] =  adc_raw[4] & 0xff;
				
					data[2] = (adc_raw[0] & 0xff00) >> 8;
					data[3] =  adc_raw[0] & 0xff;

				
					data[4] = (adc_raw[3] & 0xff00) >> 8;
					data[5] =  adc_raw[3] & 0xff;
				  ...
				  if(var)
				   transmit_string(data,11); // Transfer data when BLE Connected
